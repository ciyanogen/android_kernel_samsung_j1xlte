...
#else
			if (target_type == ID_CL0 || target_type == ID_CL1)
				return match_asv_info->asv_volt[i].asv_value + 50000;
			else
				return match_asv_info->asv_volt[i].asv_value;
#endif
#else
...
static void set_asv_info(struct asv_common *exynos_asv_common, bool show_volt)
{
	...
	list_for_each_entry(exynos_asv_info, &asv_list, node) {
		if (exynos_asv_info->ops_cal) {
			match_grp_nr = exynos_asv_info->ops_cal->get_group(exynos_asv_info->asv_type, 0);
		} else if (exynos_asv_info->ops) {
			match_grp_nr = exynos_asv_info->ops->get_asv_group(exynos_asv_common);
		} else {
			pr_err("ASV: ops is null.\n");
			BUG_ON(1);
		}
