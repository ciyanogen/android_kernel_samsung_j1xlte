...
#if defined(CONFIG_EXYNOS3475_DECON_LCD_SC7798D) || defined(CONFIG_EXYNOS3475_DECON_LCD_SC7798D_XCOVER3)
#define MDNIE_SCR_REG_REVERSE
#endif
	...
	if (ret < 0)
		...
		if (value == COLOR_BLIND) {
			...
			while (i < ARRAY_SIZE(s)) {
				wbuf[i * 2 + 0] = GET_MSB_8BIT(s[ARRAY_SIZE(s)-(i+1)]);
				wbuf[i * 2 + 1] = GET_LSB_8BIT(s[ARRAY_SIZE(s)-(i+1)]);
				i++;
			}
#else
			while (i < ret - 1) {
