...
#ifdef CONFIG_TOUCHSCREEN_MELFAS_MMS438_USE_DUAL_FW
	//power on -> wait 150~200ms -> mms_get_module_vendor()
	msleep(150);

	mms_get_module_vendor(info);
#endif
#endif
...
#ifdef CONFIG_TOUCHSCREEN_MELFAS_MMS438_USE_DUAL_FW
/*
 * F/W version    | 1    1        1     | 2   2        2     | x    x        x   |
 * H/W category   | 33   33(fake) not33 | 33  33(fake) not33 | 33   33(fake) x   |
                   --------------------------------------------------------------
 * H/W cate retry | o    o        x     | x   x        o     | o    o        x   |
 * module         | 1st  1st      1st   | 2nd 2nd      1st   | 1st  1st      1st |
 * force update   | x    x        x     | x   x        o     | x    x        x   |
 */
#ifdef CONFIG_SEC_FACTORY
int mms_get_module_vendor(struct mms_ts_info *info)
{	
	int ret = fw_err_none;
	u8 wbuf[2];
	u8 rbuf[2];
	u16 version[4];
	int fwver = 0; // 0 = Unknown, 1 = Old, 2 = New(ALL ITO)
	int hwcat = 0; // 0 = Unknown, 1 = Old, 2 = New(ALL ITO)
	int module = 0; // 0 = Unknown, 1 = Old, 2 = New(ALL ITO)
	int pass_cnt = 0;
	bool force_update_matched_fw = false;
	...
	//Get F/W version
	if (mms_get_fw_version_u16(info, version)) {
		tsp_debug_err(true, &info->client->dev, "%s [ERROR] F/W Version\n", __func__);
		fwver = 0;
	} else {
		tsp_debug_info(true, &info->client->dev, "%s - F/W Version [0x%04X]\n", __func__, version[2]);
		if ((version[2] >= 0x0116) && (version[2] < 0x0150)) {
			fwver = 1;
		} else if ((version[2] >= 0x0150) && (version[2] < 0x01FF)) {
			fwver = 2;
		} else {
			fwver = 0;
		}
	...
	if (mms_i2c_read(info, wbuf, 2, rbuf, 1)) {
		tsp_debug_err(true, &info->client->dev, "%s [ERROR] H/W Category\n", __func__);
		hwcat = 0;
	} else {
		tsp_debug_info(true, &info->client->dev, "%s - H/W Category [0x%02X]\n", __func__, rbuf[0]);
		if (rbuf[0] == 0x33) {
		        hwcat = 2;
		} else {
		        hwcat = 1;
 	...
 	if((fwver == 1 && hwcat == 2) || (fwver == 2 && hwcat == 1)) {
		if (mms_i2c_read(info, wbuf, 2, rbuf, 1)) {
			tsp_debug_err(true, &info->client->dev, "%s [ERROR] H/W Category\n", __func__);
			hwcat = 0;
		} else {
	...
	tsp_debug_info(true, &info->client->dev,
		"%s - pass[%d] hwcat[%d] module[%d] force_update_matched_fw[%d] confirmed[%d]\n",
		__func__, pass_cnt, hwcat, module, force_update_matched_fw, confirmed);

	info->pass_cnt = pass_cnt;
	info->hwcat = hwcat;
	info->module = module;
	info->force_update_matched_fw = force_update_matched_fw;
...
int mms_fw_update_from_kernel(struct mms_ts_info *info, bool force)
{
	...
	const char *fw_name_ve = info->dtdata->fw_name_ve;
...
start:
	tsp_debug_info(true, &info->client->dev, "%s - pass_cnt [%d]\n", __func__, pass_cnt);
	if (pass_cnt > 3) {
		tsp_debug_err(true, &info->client->dev, "%s [ERROR] pass_cnt limit\n", __func__);
	...
	if (module == 1) {
		...
		tsp_debug_info(true, &info->client->dev, "%s - request_firmware [%s]\n", __func__, fw_name);
	} else if (module == 2) {
		request_firmware(&fw, fw_name_ve, &info->client->dev);
		tsp_debug_info(true, &info->client->dev, "%s - request_firmware [%s]\n", __func__, fw_name_ve);
	} else {
		tsp_debug_err(true, &info->client->dev, "%s [ERROR] module type\n", __func__);
	...
	retry = 3;
	do {
		if (info->force_update_matched_fw == true) {
		...
		//failure
		tsp_debug_err(true, &info->client->dev, "%s [ERROR] retry [%d]\n", __func__, retry);
	} while (--retry);

	if (!retry) {
		tsp_debug_err(true, &info->client->dev, "%s [ERROR] retry limit\n", __func__);
		ret = fw_err_download;
	}
