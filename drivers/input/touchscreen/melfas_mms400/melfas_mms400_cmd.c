...
static void cmd_get_fw_ver_bin(void *device_data)
{
	...
	if (info->module == 1)
		...
		request_firmware(&fw, info->dtdata->fw_name_ve, &info->client->dev);
	else
		request_firmware(&fw, info->dtdata->fw_name, &info->client->dev);
#else
...
static void get_module_vendor(void *device_data)
{
	...
	switch(info->module){ // 0 = Unknown, 1 = Old, 2 = New(ALL ITO)
	case 0:
		snprintf(buf, sizeof(buf), "%s,%d", "OK", 1);
		break;
	case 1:
		snprintf(buf, sizeof(buf), "%s,%d", "OK", 1);
		break;
	case 2:
		snprintf(buf, sizeof(buf), "%s,%d", "OK", 2);
		break;
	default:
		break;
	}
	...
	tsp_debug_dbg(true, &info->client->dev, "%s - cmd[%s] len[%d] state[%d]\n",
