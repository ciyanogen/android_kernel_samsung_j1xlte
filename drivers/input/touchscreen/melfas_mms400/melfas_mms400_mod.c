...
void mms_input_event_handler(struct mms_ts_info *info, u8 sz, u8 *buf)
{
	struct i2c_client *client = info->client;
	int i;
	int module;	// 0 = Unknown, 1 = Old, 2 = New(ALL ITO)
	...
	for (i = 1; i < sz; i += info->event_size) {
		...
		if ((tmp[0] & MIP_EVENT_INPUT_SCREEN) == 0) {
			...
			tsp_debug_info(true, &client->dev, "%s - Key : ID[%d] Code[%d] State[%d]\n",
...
#endif
		{
			...
			if (info->finger_state[id] == 0){
				...
				tsp_debug_info(true, &client->dev,
					"P[%d] p:%d m:%d tc:%d module:%d\n",
					id, pressure, touch_major, info->touch_count, module);
#else
				tsp_debug_err(true, &client->dev,
					"P[%d] (%d, %d) p:%d m:%d tc:%d module:%d\n",
					id, x, y, pressure, touch_major, info->touch_count, module);
#endif
#if defined (CONFIG_INPUT_BOOSTER)
...
#ifdef CONFIG_TOUCHSCREEN_MELFAS_MMS438_USE_DUAL_FW
	if (of_property_read_string(np, "melfas,fw_name_ve", &info->dtdata->fw_name_ve)) {
		tsp_debug_err(true, dev, "Failed to get fw_name_ve property\n");
		info->dtdata->fw_name_ve = INTERNAL_FW_PATH_VE;
	}
#endif
