...
mali_error kbasep_js_devdata_init(struct kbase_device * const kbdev)
{
	...
	js_devdata->secure_ops = (struct kbase_secure_ops*)kbasep_get_config_value(kbdev, kbdev->config_attributes, KBASE_CONFIG_ATTR_SECURE_CALLBACKS);
...
void kbasep_js_runpool_requeue_or_kill_ctx(struct kbase_device *kbdev, struct kbase_context *kctx, mali_bool has_pm_ref)
{
	...
	if (kbdev->js_data.nr_all_contexts_running == 0
			...
			|| (kbdev->js_data.nr_all_contexts_running == 1 && kbdev->hwcnt.kctx != NULL)
		   	|| (kbdev->js_data.nr_all_contexts_running == 1 && kbdev->hwcnt.is_stoped)
#endif
			) {
		/* Either:
		...
		* Hence, we record going idle */
		...
		if(kbdev->vendor_callbacks->pm_record_state)
			kbdev->vendor_callbacks->pm_record_state(kbdev, MALI_FALSE);
	}
#endif
}
...
void kbasep_js_try_schedule_head_ctx(struct kbase_device *kbdev)
{
	...
	if (kbdev->hwcnt.kctx != head_kctx) {
		...
		*/
		/* MALI_SEC_INTEGRATION */
		if(kbdev->vendor_callbacks->pm_record_state)
			kbdev->vendor_callbacks->pm_record_state(kbdev, MALI_TRUE);
	}
#endif
