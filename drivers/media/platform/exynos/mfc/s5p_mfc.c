...
static void s5p_mfc_watchdog_worker(struct work_struct *work)
{
	...
	mfc_debug(2, "MFC power off to disable H/W.\n");

	mdelay(500);

	/* Power on again. Power will be off when release is called. */
	mfc_debug(2, "MFC power on again.\n");
...
static int s5p_mfc_request_sec_pgtable(struct s5p_mfc_dev *dev)
{
	...
	if (base) {
		...
		mfc_debug(2, "VIDEO_EXT region is not used.\n");
	}
...
static int s5p_mfc_open(struct file *file)
{
	...
	if (is_drm_node(node)) {
		if (dev->num_drm_inst < MFC_MAX_DRM_CTX) {
			...
			mfc_debug(2, "DRM instance is opened [%d:%d]\n",
		...
		mfc_debug(2, "NORMAL instance is opened [%d:%d]\n",
				dev->num_drm_inst, dev->num_inst);
	}
#endif
	...
	mfc_debug(2, "MFC open completed [%d:%d] dev = %p, ctx = %p\n",
...
static int s5p_mfc_release(struct file *file)
{
	...
	mfc_debug(2, "MFC driver release is called [%d:%d], is_drm(%d)\n",
	...
	mfc_debug(2, "mfc driver release finished [%d:%d], dev = %p\n",
...
static void parse_mfc_qos_mif_update(struct s5p_mfc_dev *dev, struct s5p_mfc_qos *pdata, int num_steps)
{
	...
	if (num_steps > MIF_MAX_COUNT) {
		...
		mfc_debug(2, "MIF_MAX_COUNT = %d, steps for dev-%d : %d\n",
...
static int s5p_mfc_probe(struct platform_device *pdev)
{
	...
	*vfd = s5p_mfc_enc_drm_videodev;
	...
	for (i = 0; i < dev->pdata->num_qos_steps; i++) {
		mfc_debug(2, "QoS table[%d] int : %d (mfc : %d), mif : %d\n",
				i,
		...
		if (dev->pdata->qos_extra[i].thrd_mb != MFC_QOS_FLAG_NODATA)
			mfc_debug(2, ">> Extra[%d] int : %d (mfc : %d), mif : %d\n",
				i,
