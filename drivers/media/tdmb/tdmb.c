...
static int tdmb_vreg_init(struct device *dev)
{
	...
	if (dt_pdata->tdmb_vreg_name2) {
		DPRINTK("vdd2_name : %s", dt_pdata->tdmb_vreg_name2);
		dt_pdata->tdmb_vreg2 = regulator_get(dev, dt_pdata->tdmb_vreg_name2);
		if (IS_ERR(dt_pdata->tdmb_vreg2)) {
			DPRINTK("Failed to get tdmb_vreg\n");
			rc = -ENXIO;
			return rc;
		}
		rc = regulator_set_voltage(dt_pdata->tdmb_vreg2, 1800000, 1800000);
		if (rc) {
			DPRINTK("regulator set_vtg2 failed rc=%d\n", rc);
			regulator_put(dt_pdata->tdmb_vreg2);
			return rc;
		}
	}
...
static void tdmb_vreg_onoff(bool onoff)
{
	...
	if (onoff) {
		...
		if (dt_pdata->tdmb_vreg_name2) {
			if (!regulator_is_enabled(dt_pdata->tdmb_vreg2)) {
				rc = regulator_enable(dt_pdata->tdmb_vreg2);
				if (rc)
					DPRINTK("tdmb_vreg2 enable failed rc=%d\n", rc);
			}
		}
	} else {
		if (dt_pdata->tdmb_vreg_name) {
		...
		if (dt_pdata->tdmb_vreg_name2) {
			if (regulator_is_enabled(dt_pdata->tdmb_vreg2)) {
				rc = regulator_disable(dt_pdata->tdmb_vreg2);
				if (rc)
					DPRINTK("tdmb_vreg2 disable failed rc=%d\n", rc);
			}
		}
	}
...
static void tdmb_gpio_on(void)
{
	...
	if (dt_pdata->tdmb_fm_ant_sel)
		gpio_set_value(dt_pdata->tdmb_fm_ant_sel, 1);

        if (dt_pdata->tdmb_use_expander_gpio)
...
static void tdmb_gpio_off(void)
{
	...
	if (dt_pdata->tdmb_fm_ant_sel)
		gpio_set_value(dt_pdata->tdmb_fm_ant_sel, 0);
...
static int tdmb_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (size > TDMB_RING_BUFFER_MAPPING_SIZE) {
		DPRINTK("over size given : %u\n", size);
		return -EAGAIN;
	}
	DPRINTK("size given : %u\n", size);

#if TDMB_PRE_MALLOC
	size = TDMB_RING_BUFFER_MAPPING_SIZE;
	if (!ts_ring) {
		DPRINTK("RING Buff ReAlloc(%u)!!\n", size);
#endif
		/* size should aligned in PAGE_SIZE */
		if (size % PAGE_SIZE) /* klaatu hard coding */
			size = size + size % PAGE_SIZE;
...
static struct tdmb_dt_platform_data *get_tdmb_dt_pdata(struct device *dev)
{
	...
	if(of_property_read_string(dev->of_node,
		...
		pdata->tdmb_vreg_name = NULL;
		/* goto alloc_err; */
	}

	if(of_property_read_string(dev->of_node,
			"tdmb_vreg_supply2", &pdata->tdmb_vreg_name2)) {
		DPRINTK("Unable to find tdmb_vdd_supply2\n");
		pdata->tdmb_vreg_name2 = NULL;
		/* goto alloc_err; */
	}
#endif

        pdata->tdmb_fm_ant_sel = of_get_named_gpio(dev->of_node, "tdmb_fm_ant_sel", 0);
        if (!gpio_is_valid(pdata->tdmb_fm_ant_sel)) {
                DPRINTK("%s : GPIO tdmb_fm_ant_sel is invalid\n", __func__);
		pdata->tdmb_fm_ant_sel = 0;
        }

	 pdata->tdmb_use_expander_gpio = of_property_read_bool(dev->of_node, "tdmb_use_expander_gpio");
